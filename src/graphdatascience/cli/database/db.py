...
Reuses :class:`graphdatascience.cli.common.env.DatabaseConfig` (built from the
...
from graphdatascience.cli.common.env import DatabaseConfig, database_config_from_env
...
class DatabaseClient:
    ...
    def upload(self, graph: Graph, *, overwrite: bool = False, batch_size: int = 5_000) -> None:
        from graphdatascience.cli.database.upload import upload_graph

        upload_graph(graph, self.db_config, batch_size=batch_size, overwrite=overwrite)

    def fetch(self, node_labels: list[str] | None = None) -> Graph:
        from graphdatascience.cli.database.fetch import download_graph
