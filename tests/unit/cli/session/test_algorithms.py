...
from graphdatascience.cli.session.algorithms import resolve_endpoint, run_algorithm
from graphdatascience.cli.session.config import AlgorithmConfig
...
def test_run_algorithm_mutate_mode_calls_mutate() -> None:
    gds = MagicMock()
    graph = MagicMock()
    algo = AlgorithmConfig(name="louvain", mode="mutate", mutate_property="community", parameters={"maxIterations": 5})
...
def test_run_algorithm_write_mode_calls_write() -> None:
    gds = MagicMock()
    graph = MagicMock()
    algo = AlgorithmConfig(name="pageRank", mode="write", write_property="rank")
