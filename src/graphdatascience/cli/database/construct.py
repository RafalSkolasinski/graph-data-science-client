...
``nodeId -> elementId`` in a single dict — see :mod:`graphdatascience.cli.database.examples`).

Example (matches the FastPath "direct" notebook)::

    import pandas as pd
    from graphdatascience.cli.database.construct import graph_from_construct_format
...
def graph_from_file(path: str | Path) -> Graph:
    ...
    * ``kind: random`` -> generated from a random-graph spec (see
      :mod:`graphdatascience.cli.database.spec`).
    """
    ...
    if kind == "random":
        from graphdatascience.cli.database.spec import graph_from_spec
