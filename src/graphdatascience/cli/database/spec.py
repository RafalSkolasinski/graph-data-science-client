...
generator, and property generators. It is the file-driven counterpart of the
built-in example builders in :mod:`graphdatascience.cli.database.examples`.
...
Relationship generator ``type``: uniform | powerlaw (with ``alpha``). Both edge
generators accept ``allow_duplicates: true``.
"""
...
def _property_generator(cfg: dict[str, Any]) -> RandomGenerator:
    t = cfg.get("type")
    if t == "gaussian":
        return GaussianGenerator(mean=cfg.get("mean", 0.0), std=cfg.get("std", 1.0))
    if t == "uniform_int":
        return UniformIntegerGenerator(low=cfg["low"], high=cfg["high"])
    if t == "category":
