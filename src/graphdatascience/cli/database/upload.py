...
from graphdatascience.cli.common.env import DatabaseConfig
from graphdatascience.cli.database.db import DEFAULT_EXTRA_LABEL, delete_graph, graph_exists
    ...
    with GraphDatabase.driver(db_config.uri, auth=db_config.auth) as driver:
        with driver.session(database=db_config.database) as session:
            ...
            for label, df in graph.node_dfs.items():
                ...
                with tqdm(total=len(records), desc=f"Nodes (:{label})", unit="node") as pbar:
            ...
            for (src_label, rel_type, tgt_label), df in graph.rel_dfs.items():
                ...
                with tqdm(total=len(rels), desc=desc, unit="edge") as pbar:
